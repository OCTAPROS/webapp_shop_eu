...
def create_product(product_data: ProductInsert, db_session: Session = Depends(get_session)):
    product = Product(**product_data.model_dump())
    db_session.add(product)  
    db_session.commit()
    db_session.refresh(product)
    
    productM: ProductView = db_session.exec(select(ProductView).where(ProductView.id == product.id)).one_or_none()
    if not product:
        raise HTTPException(detail=f"Product with id {id} does not exist", status_code=status.HTTP_404_NOT_FOUND)
    return productM

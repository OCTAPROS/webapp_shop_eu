...
def get_all_products(skip: int = 0, limit = 10, db_session: Session = Depends(get_dbsession)):
...
def get_product_by_id(id:int, db_session: Session = Depends(get_dbsession)):
...
def create_product(product: Product, db_session: Session = Depends(get_dbsession)):
...
def update_product(id:int, product_data: Product, db_session: Session = Depends(get_dbsession)):
    product = db_session.get(Product, id)
    if not product:
        raise HTTPException(detail=f"Product with id {id} does not exist", status_code=status.HTTP_404_NOT_FOUND)
...
def delete_product(id:int, db_session: Session = Depends(get_dbsession)):
    product = db_session.get(Product, id)
    if not product:
        raise HTTPException(detail=f"Product with id {id} does not exist", status_code=status.HTTP_404_NOT_FOUND)
